...
async function sendSummary(session) {
  ...
  // Gửi cho người tạo phiên và những người được chia sẻ phiên (đều quản lý phiên như nhau)
  const { rows: shared } = await query(
    `SELECT u.email FROM session_shares s JOIN users u ON u.id = s.user_id
     WHERE s.session_id = $1 AND u.email IS NOT NULL AND u.email <> ''`, [session.id]);
  const to = [...new Set([owner && owner.email, ...shared.map((r) => r.email)].filter(Boolean))];
  if (!to.length) return;
  ...
  await sendMail({
    ...
    html: emailLayout('Kết thúc điểm danh', `
      <p>Xin chào,</p>
      <p>Phiên điểm danh <b>${session.name}</b> (người tạo: <b>${owner ? (owner.full_name || owner.username) : '—'}</b>)
         đã tự động kết thúc lúc <b>${session.closed_at}</b> theo thời gian đã hẹn.</p>
